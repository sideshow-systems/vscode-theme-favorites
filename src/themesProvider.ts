...
export class ThemesProvider implements vscode.TreeDataProvider<ThemeNode> {
	private _onDidChangeTreeData: vscode.EventEmitter<ThemeNode | undefined | void> =
		new vscode.EventEmitter();
	readonly onDidChangeTreeData: vscode.Event<ThemeNode | undefined | void> =
	...
	async getChildren(element?: ThemeNode): Promise<ThemeNode[]> {
		...
		function determineKind(entry: {
			...
			try {
				const bg =
					entry.colors &&
					(entry.colors['editor.background'] || entry.colors['editorBackground']);
			...
			if (lab.includes('dark') || lab.includes('frappe') || lab.includes('mocha'))
				return 'dark';
		...
		if (element instanceof GroupItem) {
			...
			const items = all
				.filter((t) => determineKind(t) === element.kind)
				.sort((a, b) => a.label.localeCompare(b.label));
			return items.map((i) => {
				const item = new ThemeItem(
					i.label,
					favs.includes(i.label),
					isSameTheme(i.label, active)
				);
	...
	> {
		...
		for (const ext of vscode.extensions.all) {
			...
			for (const t of themes) {
				...
				const uiTheme =
					typeof t === 'object' ? (t.uiTheme as string | undefined) : undefined;
				if (!label) continue;
				if (!set.has(label)) {
					const entry: {
						label: string;
